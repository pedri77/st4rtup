  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", color: T.fg, minHeight: '100vh', backgroundColor: T.bg }}>
      ...
      <nav style={{ backgroundColor: T.card, borderBottom: `1px solid ${T.border}`, padding: '0 24px', height: 80, display: 'flex', alignItems: 'center', justifyContent: 'space-between', maxWidth: 1200, margin: '0 auto', width: '100%' }}>
        ...
        <Link to="/" style={{ fontSize: 14, color: T.primary, textDecoration: 'none', fontWeight: 500 }}>← Volver</Link>
      </nav>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '60px 24px 100px' }}>
        ...
        <p style={{ color: T.fgMuted, fontSize: 16, marginBottom: 32 }}>Disponibilidad en tiempo real</p>
        ...
        <div style={{ backgroundColor: T.card, borderRadius: 14, border: `1px solid ${T.border}`, overflow: 'hidden' }}>
          {SERVICES.map((svc, i) => {
            ...
            return (
              <div key={svc.name} style={{ padding: '16px 20px', borderBottom: i < SERVICES.length - 1 ? `1px solid ${T.muted}` : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <span style={{ fontSize: 12, color: T.fgMuted, fontFamily: fontMono }}>{svc.uptime}%</span>
                </div>
              </div>
            )
          })}
        ...
        <div style={{ marginTop: 32, backgroundColor: T.card, borderRadius: 14, border: `1px solid ${T.border}`, padding: 20 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: T.fgMuted, marginTop: 6, fontFamily: fontMono }}>
            <span>90 días</span>
            <span>Hoy</span>
          </div>
        </div>
        ...
        <div style={{ marginTop: 32, backgroundColor: T.card, borderRadius: 14, border: `1px solid ${T.border}`, padding: 20 }}>
          ...
          <p style={{ color: T.fgMuted, fontSize: 14 }}>Sin incidentes en los últimos 30 días.</p>
        </div>

        {/* Subscribe */}
        <div style={{ marginTop: 32, padding: '24px 20px', borderRadius: 14, backgroundColor: T.card, border: `1px solid ${T.border}`, textAlign: 'center' }}>
          <p style={{ fontWeight: 600, fontSize: 14, marginBottom: 12 }}>Recibe alertas de estado</p>
          <div style={{ display: 'flex', gap: 8, maxWidth: 360, margin: '0 auto' }}>
            <input placeholder="tu@email.com" style={{ flex: 1, padding: '10px 14px', borderRadius: 8, border: `1px solid ${T.border}`, fontSize: 13, backgroundColor: T.muted, color: T.fg }} />
            <button style={{ padding: '10px 18px', borderRadius: 8, border: 'none', backgroundColor: T.primary, color: 'white', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>Suscribir</button>
          </div>
        </div>
      </div>
    <WebChatWidget />
