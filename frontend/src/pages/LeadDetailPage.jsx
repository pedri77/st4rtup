...
export default function LeadDetailPage() {
  ...
  const enrichMutation = useMutation({
    ...
    onSuccess: (res) => {
      const data = res.data;
      if (data.enriched) {toast.success(`Lead enriquecido via ${data.source}`);refetch();} else
      toast.error(data.error || 'No se encontraron datos de enriquecimiento');
    },
      ...
      <div className="mb-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-2">
            ...
            style={{ fontFamily: fontDisplay, backgroundColor: T.cyan, color: T.bg }} aria-label="Enviar email">
              <Mail className="w-4 h-4" />
              {triggerEM01.isPending ? 'Enviando...' : 'Iniciar Welcome Sequence (EM-01)'}
            </button>
            <Link to={`/app/leads/${id}/abm`}
