    ...
    <div className="rounded-lg transition-all" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
      <div className="flex items-center gap-0 cursor-pointer p-4" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-1">
            <span className="text-sm font-semibold truncate" style={{ color: T.fg }}>
              {visit.lead_id ?
              <Link to={`/app/leads/${visit.lead_id}`} className="transition-colors" style={{ color: T.fg }} onClick={(e) => e.stopPropagation()}>
                  {visit.lead_name || '—'}
                </Link> :
              visit.lead_name || '—'}
            </span>
        ...
        <div className="flex items-center gap-1 px-3 flex-shrink-0">
          <button onClick={(e) => {e.stopPropagation();onDelete(visit.id);}} className="p-1.5 transition-colors" style={{ color: T.fgMuted }} aria-label="Cerrar">
            <X className="w-4 h-4" />
          </button>
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4 overflow-y-auto" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
      <div className="rounded-lg max-w-3xl w-full my-8" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
        <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${T.border}` }}>
          ...
          <button onClick={onClose} style={{ color: T.fgMuted }} aria-label="Cerrar"><X className="w-5 h-5" /></button>
        </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  ...
                  <span key={idx} className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs" style={{ color: T.cyan, backgroundColor: `${T.cyan}15` }}>
                        {name} <button type="button" onClick={() => removeFromList('attendees_internal', idx)} aria-label="Cerrar"><X className="w-3 h-3" /></button>
                      </span>
                  )}
                  </div>
                }
              ...
              <div>
                ...
                <div className="space-y-1 mt-2">
                  ...
                  <div key={idx} className="flex items-center justify-between p-2 rounded" style={{ backgroundColor: T.muted }}>
                        ...
                        <button type="button" onClick={() => removeFromList('attendees_external', idx)} aria-label="Cerrar"><X className="w-3 h-3" style={{ color: T.fgMuted }} /></button>
                      </div>
                  )}
                  </div>
                }
