  ...
  if (isError || !stats) {
    return (
      <div className="card border border-gray-200/50">
        ...
        <Link to="/app/marketing" className="inline-flex items-center gap-1 text-xs text-cyan-400 hover:text-cyan-300 font-medium mt-2">
  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/app/marketing" className="flex items-center gap-1 text-xs text-cyan-400 hover:text-cyan-300 font-medium">
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        <div className="bg-gray-50 rounded-lg p-3 text-center">
          <p className="text-xl font-bold text-gray-800">{campaigns.total || 0}</p>
        ...
        <div className={clsx('rounded-lg p-3 text-center', counts.alerts_unread > 0 ? 'bg-yellow-50' : 'bg-gray-50')}>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="bg-gray-50 rounded-lg p-3">
          ...
          <p className="text-sm font-bold text-gray-800">{assets.total || 0} activos</p>
          <p className="text-[10px] text-gray-500 mt-0.5">
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {channels.map(([channel, count]) => (
              <span key={channel} className="inline-flex items-center gap-1 text-[10px] bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full">
                {channelLabels[channel] || channel} <span className="font-bold text-gray-800">{count}</span>
              </span>
            ))}
          </div>
        </div>
      ...
      {/* Module counts */}
      <div className="flex flex-wrap gap-1.5">
        <span className="inline-flex items-center gap-1 text-[10px] bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full">
          <Target className="w-2.5 h-2.5" /> Funnels <span className="font-bold text-gray-800">{counts.funnels || 0}</span>
        </span>
        <span className="inline-flex items-center gap-1 text-[10px] bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full">
          <FileText className="w-2.5 h-2.5" /> UTMs <span className="font-bold text-gray-800">{counts.utm_codes || 0}</span>
        </span>
        <span className="inline-flex items-center gap-1 text-[10px] bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full">
          <BarChart3 className="w-2.5 h-2.5" /> Eventos <span className="font-bold text-gray-800">{counts.calendar_events || 0}</span>
        </span>
      </div>
    </div>
  )
